...
import (
	...
	"github.com/ben-han-cn/kvzoo/client"
	"github.com/ben-han-cn/kvzoo/server"
)
...
func testList(t *testing.T, db kvzoo.DB) {
	keys, values := genData("key", "value", 1000)
	tableName, _ := kvzoo.NewTableName("/views/v1")
	err := loadDataToTable(db, tableName, keys, values)
	ut.Equal(t, err, nil)
	data, err := getTableData(db, tableName)
	ut.Equal(t, err, nil)
	...
	keys, values = genData("key", "value", 10)
	subTableName, _ := kvzoo.NewTableName("/views/v1/zones")
	loadDataToTable(db, subTableName, keys, values)
	data, _ = getTableData(db, tableName)
	ut.Equal(t, len(data), 1000)
}
...
func TestBoltDBTables(t *testing.T) {
	withBoltDB(t, testTables)
}

func TestRemoteDBTables(t *testing.T) {
	withRemoteDB(t, testTables)
}

func testTables(t *testing.T, db kvzoo.DB) {
	tables, err := db.Tables()
	ut.Equal(t, err, nil)
	ut.Assert(t, tables == nil, "")

	tn, _ := kvzoo.NewTableName("/views/v1")
	keys, values := genData("k", "v", 10)
	loadDataToTable(db, tn, keys, values)

	tn, _ = kvzoo.NewTableName("/user")
	db.CreateOrGetTable(tn)
	tables, err = db.Tables()
	ut.Equal(t, err, nil)
	ut.Equal(t, tables, []string{"user", "views"})

	tn, _ = kvzoo.NewTableName("/views")
	table, _ := db.CreateOrGetTable(tn)
	tx, _ := table.Begin()
	tables, err = tx.Tables()
	tx.Rollback()
	ut.Equal(t, err, nil)
	ut.Equal(t, tables, []string{"v1"})

	tx, _ = table.Begin()
	err = tx.Add("v1", []byte{1})
	ut.Assert(t, err != nil, "")
	tx.Rollback()
