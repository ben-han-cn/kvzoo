...
import (
	...
	pb "github.com/ben-han-cn/kvzoo/proto"
)
...
func (p *Proxy) Tables() ([]string, error) {
	req := &pb.GetTablesRequest{}
	reply, err := p.master.Tables(context.TODO(), req)
	if err != nil {
		return nil, err
	}
...
func (tx *ProxyTransaction) Tables() ([]string, error) {
	req := &pb.GetTablesRequest{
		TxId: tx.ids[0],
	}
	reply, err := tx.proxy.master.Tables(context.TODO(), req)
	if err != nil {
		return nil, err
	}
	return reply.Tables, nil
